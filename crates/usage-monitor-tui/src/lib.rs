...
use widgets::{format_tokens, format_cost, get_provider_shade, get_provider_from_model};
...
// ── Data ──
struct ModelRow { model_id: String, provider: String, input: i64, output: i64, cache_read: i64, cache_write: i64, sessions: usize, requests: usize, cost: f64 }
...
struct App {
    tab: usize, scroll: usize, selected: usize, sort_col: u8, sort_desc: bool,
    last_refresh: Instant,
    models: Vec<ModelRow>, daily: Vec<DailyRow>, sessions: Vec<SessRow>, heatmap: Vec<(String,i64)>,
    msg_count: i64, total_cost: f64,
...
impl App {
    ...
    fn reload(&mut self, storage: &Storage, pricing: &PricingEngine) {
        ...
        for (i,(name,_)) in ranked.iter().enumerate() {
            let p = get_provider_from_model(name);
            self.model_shade.insert(name.clone(), (get_provider_shade(p, i/5), i));
        }

        self.models = ms.iter().map(|x|{
            let (color,_)=self.model_shade.get(&x.model_id).copied().unwrap_or((Color::Gray,0));
            ModelRow{model_id:x.model_id.clone(),provider:x.provider_id.clone(),
    ...
    fn sort(&mut self){match self.sort_col{
    ...
    fn scroll_to(&mut self){if self.selected<self.scroll{self.scroll=self.selected}if self.selected>=self.scroll.saturating_add(18){self.scroll=self.selected.saturating_sub(17)}}
}
...
pub fn run(db_path: PathBuf, home: PathBuf) -> anyhow::Result<()> {
    ...
    let backend=CrosstermBackend::new(stdout);let mut t=Terminal::new(backend)?;
    let res=event_loop(&mut t,&mut app,&storage,&pricing);
    disable_raw_mode()?;execute!(t.backend_mut(),LeaveAlternateScreen,DisableMouseCapture)?;t.show_cursor()?;res
}

fn event_loop<B:Backend>(t:&mut Terminal<B>,app:&mut App,storage:&Storage,pricing:&PricingEngine)->anyhow::Result<()>{
    loop{t.draw(|f|ui(f,app))?;
        if event::poll(Duration::from_millis(250))?{match event::read()?{
            Event::Key(k) if k.kind!=KeyEventKind::Release=>{if handle_key(app,k.code){return Ok(());}}
            Event::Mouse(m)=>match m.kind{MouseEventKind::ScrollDown=>{app.scroll+=3;app.selected+=3}MouseEventKind::ScrollUp=>{app.scroll=app.scroll.saturating_sub(3);app.selected=app.selected.saturating_sub(3)}_=>{}},_=>{}}}
        if app.last_refresh.elapsed().as_secs()>=REFRESH_SECS{app.reload(storage,pricing);app.last_refresh=Instant::now();}}
}

fn handle_key(app:&mut App,key:KeyCode)->bool{match key{
...
fn render_top_models_tokscale_style(f:&mut Frame,area:Rect,app:&App){
    let total:f64=app.models.iter().map(|m|if m.cost.is_finite(){m.cost}else{0.0}).sum::<f64>().max(0.01);
    let items_per_page=((area.height.saturating_sub(2)/2)as usize).max(1);
    let block=Block::default().borders(Borders::ALL).border_style(Style::default().fg(BORDER))
        .title(Span::styled(" Models by Cost ",Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)))
        .title_top(Line::from(Span::styled(format!(" Total: {} ",format_cost(app.total_cost)),Style::default().fg(Color::Green))).right_aligned())
        .style(Style::default().bg(BG));
    let inner=block.inner(area);f.render_widget(block,area);
    if app.models.is_empty(){f.render_widget(Paragraph::new("No data").style(Style::default().fg(MUTED)).alignment(Alignment::Center),inner);return;}

    let start=app.scroll.min(app.models.len());
    let end=(start+items_per_page).min(app.models.len());
    let mut y=inner.y;
    for i in start..end{
        if y+1>=inner.y+inner.height{break}
        let m=&app.models[i];let is_sel=i==app.selected;
        let (color,_)=app.model_shade.get(&m.model_id).copied().unwrap_or((Color::Gray,0));
        let name=trunc(&m.model_id,30);
        let pct=if m.cost.is_finite()&&total>0.0{(m.cost/total)*100.0}else{0.0};
        ...
        let l2=Line::from(vec![
            Span::styled("  In: ",Style::default().fg(DIM)),Span::styled(format_tokens(m.input as u64),Style::default().fg(Color::Rgb(170,170,170))),
            Span::styled(" · Out: ",Style::default().fg(DIM)),Span::styled(format_tokens(m.output as u64),Style::default().fg(Color::Rgb(170,170,170))),
            Span::styled(" · CR: ",Style::default().fg(DIM)),Span::styled(format_tokens(m.cache_read as u64),Style::default().fg(Color::Rgb(170,170,170))),
            Span::styled(" · CW: ",Style::default().fg(DIM)),Span::styled(format_tokens(m.cache_write as u64),Style::default().fg(Color::Rgb(170,170,170))),
        ]);
        f.render_widget(Paragraph::new(l2).style(s),Rect::new(inner.x,y,inner.width,1));y+=1;
    }
    if app.models.len()>items_per_page{
        let mut ss=ScrollbarState::new(app.models.len()).position(app.scroll);
        f.render_stateful_widget(Scrollbar::new(ScrollbarOrientation::VerticalRight).begin_symbol(Some("▲")).end_symbol(Some("▼")).track_symbol(Some("│")).thumb_symbol("█"),inner.inner(Margin{horizontal:0,vertical:0}),&mut ss);
    }
}
...
fn tab_models(f:&mut Frame,area:Rect,app:&App){
    ...
    let rows:Vec<Row>=app.models[app.scroll..end].iter().map(|m|{let(c,_)=app.model_shade.get(&m.model_id).copied().unwrap_or((Color::Gray,0));Row::new(vec![
        Cell::from(format!("● {}",m.model_id)).style(Style::default().fg(c)),
    ...
    f.render_widget(Table::new(rows,w).header(Row::new(vec!["Model","Sess","Tokens","Cache","Cost"]).style(Style::default().fg(Color::Cyan)))
        .block(Block::default().borders(Borders::ALL).title(format!(" Models ({}) s:sort ",app.models.len()))),area);
}
...
fn tab_daily(f:&mut Frame,area:Rect,app:&App){
    ...
    let rows:Vec<Row>=app.daily.iter().rev().skip(app.scroll).take(22).map(|d|{
        let is=d.date==today;
        Row::new(vec![Cell::from(d.date.clone()).style(if is{Style::default().fg(Color::Yellow).bold()}else{Style::default()}),
            ...
            Cell::from(format_tokens(d.cache_write as u64)),Cell::from(format!("{}",d.requests)),Cell::from(format_cost(d.cost))])
    }).collect();
    ...
    f.render_widget(Table::new(rows,w).header(Row::new(vec!["Date","Input","Output","Cache R","Cache W","Reqs","Cost"]).style(Style::default().fg(Color::Cyan)))
...
fn tab_stats(f:&mut Frame,area:Rect,app:&App){
    ...
    let cards=Layout::horizontal([Constraint::Ratio(1,4);4]).split(chunks[0]);
    let mut dates:Vec<String>=app.heatmap.iter().map(|(d,_)|d.clone()).collect();dates.sort();dates.dedup();
    let today=chrono::Local::now().format("%Y-%m-%d").to_string();
    let streak=calc_streak(&dates,&today);let longest=calc_longest(&dates);let active=dates.len();
    let total_days=if let(Some(f),Some(l))=(dates.first(),dates.last()){(chrono::NaiveDate::parse_from_str(l,"%Y-%m-%d").unwrap()-chrono::NaiveDate::parse_from_str(f,"%Y-%m-%d").unwrap()).num_days()as usize+1}else{1};
    for(i,(v,l,c))in[(format!("{} days",streak),"Current Streak",Color::Cyan),(format!("{} days",longest),"Longest Streak",Color::Yellow),(format!("{}/{}",active,total_days),"Active Days",Color::White),(format_cost(app.total_cost),"Total Cost",Color::Green)].iter().enumerate(){
        f.render_widget(Paragraph::new(vec![Line::from(Span::styled(l.to_string(),Style::default().fg(MUTED))),Line::from(Span::styled(v.clone(),Style::default().fg(*c).bold()))]).block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(BORDER))),cards[i]);
    }
    if !app.heatmap.is_empty(){
        let by:HashMap<String,i64>=app.heatmap.iter().cloned().collect();let mv=by.values().max().copied().unwrap_or(1).max(1);
        let cols=26;let end=chrono::Local::now().date_naive();let start=end-chrono::Duration::days(cols*7-1);
        let mut lines=vec![];
        for row in 0..7{let mut spans=vec![];
            for col in 0..cols{let ds=(start+chrono::Duration::days((col*7+row)as i64)).format("%Y-%m-%d").to_string();let v=by.get(&ds).copied().unwrap_or(0);
                let(ch,c)=if v==0{('·',Color::Rgb(22,27,34))}else if v<mv/3{('░',Color::Rgb(14,68,41))}else if v<mv*2/3{('▒',Color::Rgb(0,109,50))}else{('▓',Color::Rgb(57,211,83))};
                spans.push(Span::styled(format!("{} ",ch),Style::default().fg(c)));}lines.push(Line::from(spans));}
        f.render_widget(Paragraph::new(lines).block(Block::default().borders(Borders::ALL).title(" Contribution Heatmap ")),chunks[1]);
    }
}

fn calc_streak(dates:&[String],today:&str)->usize{let mut s=0;let mut d=chrono::NaiveDate::parse_from_str(today,"%Y-%m-%d").unwrap();loop{if dates.contains(&d.format("%Y-%m-%d").to_string()){s+=1;d-=chrono::Duration::days(1)}else{break}}s}
fn calc_longest(dates:&[String])->usize{let mut sorted:Vec<chrono::NaiveDate>=dates.iter().filter_map(|d|chrono::NaiveDate::parse_from_str(d,"%Y-%m-%d").ok()).collect();sorted.sort();sorted.dedup();let mut l=0;let mut c=0;let mut p:Option<chrono::NaiveDate>=None;for d in sorted{if let Some(pp)=p{if d==pp+chrono::Duration::days(1){c+=1}else{c=1}}else{c=1};l=l.max(c);p=Some(d)}l}
fn trunc(s:&str,max:usize)->String{if s.chars().count()<=max{s.to_string()}else if max<=1{"…".into()}else{format!("{}…",s.chars().take(max-1).collect::<String>())}}
